...
if not df.empty:
    ...
    def first_chart(df):
        ...
        fig = chart.show_bar_chart(show_columns)
        st.pyplot(fig)
    ...
    def second_chart(df):
        ...
        chart = GraphGenerator(df)
        fig = chart.show_bar_chart(show_columns,fontsize=12)
        st.pyplot(fig)

    def third_chart(df):
        show_columns =["Měsíc","Bílkoviny[g]_na_den_[kcal]","Komplexní_sacharidy_na_den[g]_[kcal]","Cukry[g]_na_den_[kcal]","Nasycené_mastné_kyseliny[g]_na_den_[kcal]","Nenasycené_mastné_kyseliny_na_den[g]_[kcal]"]
        chart = GraphGenerator(df)
        fig = chart.show_bar_chart(show_columns,convert_to_percentages = True,fontsize=12)
        st.pyplot(fig)
